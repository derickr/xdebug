...
static int attach_context_vars(xdebug_xml_node *node, xdebug_var_export_options *options, long context_id, long depth, void (*func)(void *, xdebug_hash_element*, void*))
{
	...
	if (EG(current_execute_data)->opline->opcode == ZEND_EXT_STMT && EG(current_execute_data)->opline->op1_type == IS_VAR) {
		xdebug_xml_node *tmp_node;
		xdebug_str *name = xdebug_str_create_from_const_char("$"XDEBUG_PIPE_VALUE_VAR_NAME);
		...
		xdebug_xml_expand_attribute_value(tmp_node, "facet", "readonly pipe_intermediate_value virtual");
