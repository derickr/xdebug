...
if test "$PHP_XDEBUG" != "no"; then
  ...
  XDEBUG_RECORDER_SOURCES="src/recorder/recorder.c src/recorder/section.c src/recorder/var_export_binary.c"
  ...
  PHP_NEW_EXTENSION(xdebug, xdebug.c $XDEBUG_BASE_SOURCES $XDEBUG_LIB_SOURCES $XDEBUG_COVERAGE_SOURCES $XDEBUG_DEBUGGER_SOURCES $XDEBUG_DEVELOP_SOURCES $XDEBUG_GCSTATS_SOURCES $XDEBUG_PROFILER_SOURCES $XDEBUG_RECORDER_SOURCES $XDEBUG_TRACING_SOURCES, $ext_shared,,$PHP_XDEBUG_CFLAGS,,yes)
  ...
  PHP_ADD_BUILD_DIR(PHP_EXT_BUILDDIR(xdebug)[/src/recorder])
